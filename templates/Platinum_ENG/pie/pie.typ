...
=======
  group(name: "chart", {
    ...
    for phylum in data {
      ...
      anchor(phylum.name, (offset + seg / 2, 3))
      if last < 30deg {
        content((offset + seg / 2 + 10deg, 5.5), align(center, {phylum.name + "\n" + str(phylum.abundance)+"%"}), name: "legend")
      }
      else {
      content((offset + seg / 2, 5.5), align(center, {phylum.name + "\n" + str(phylum.abundance)+"%"}), name: "legend")
      }
      line(phylum.name, "legend", stroke:gray)
      last = seg
      offset += seg
...
>>>>>>> 2ab8a58 (simplified code with "PIE" mode of arc)
